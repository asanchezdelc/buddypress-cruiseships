 ...
 *
 * This function is called in two places in the bp_ships_Group class:
 ...
 */
function bp_ships_delete_group_cache( $group_id = 0 ) {
	wp_cache_delete( $group_id, 'bp_groups' );
}
add_action( 'groups_delete_group',     'bp_ships_delete_group_cache' );
add_action( 'groups_update_group',     'bp_ships_delete_group_cache' );
add_action( 'groups_details_updated',  'bp_ships_delete_group_cache' );
add_action( 'groups_settings_updated', 'bp_ships_delete_group_cache' );
 ...
 */
function bp_ships_delete_group_cache_on_metadata_change( $meta_id, $group_id ) {
	wp_cache_delete( $group_id, 'bp_groups' );
}
add_action( 'updated_group_meta', 'bp_ships_delete_group_cache_on_metadata_change', 10, 2 );
add_action( 'added_group_meta', 'bp_ships_delete_group_cache_on_metadata_change', 10, 2 );
 ...
 * @param bp_ships_Group $group_obj Group object.
 */
function bp_ships_clear_group_creator_cache( $group_id, $group_obj ) {
	// Clears the 'total groups' for this user
	ships_clear_group_user_object_cache( $group_obj->id, $group_obj->creator_id );
}
add_action( 'groups_created_group', 'bp_ships_clear_group_creator_cache', 10, 2 );
 ...
 */
function bp_ships_clear_group_members_caches( $group_obj, $user_ids ) {
	// Clears the 'total groups' cache for each member in a group
	foreach ( (array) $user_ids as $user_id )
		ships_clear_group_user_object_cache( $group_obj->id, $user_id );
}
add_action( 'bp_ships_delete_group', 'bp_ships_clear_group_members_caches', 10, 2 );
 ...
 */
function bp_ships_clear_invite_count_for_user( $user_id ) {
	wp_cache_delete( $user_id, 'bp_group_invite_count' );
}
add_action( 'groups_accept_invite', 'bp_ships_clear_invite_count_for_user' );
add_action( 'groups_reject_invite', 'bp_ships_clear_invite_count_for_user' );
add_action( 'groups_delete_invite', 'bp_ships_clear_invite_count_for_user' );
 ...
 * @param int $user_id The user ID.
 */
function bp_ships_clear_invite_count_on_uninvite( $group_id, $user_id ) {
	bp_ships_clear_invite_count_for_user( $user_id );
}
add_action( 'groups_uninvite_user', 'bp_ships_clear_invite_count_on_uninvite', 10, 2 );
 ...
 */
function bp_ships_clear_invite_count_on_send( $group_id, $invited_users ) {
	foreach ( $invited_users as $user_id ) {
		bp_ships_clear_invite_count_for_user( $user_id );
	}
}
add_action( 'groups_send_invites', 'bp_ships_clear_invite_count_on_send', 10, 2 );
 ...
 * @param int $user_id The user ID.
 */
function ships_clear_group_user_object_cache( $group_id, $user_id ) {
	wp_cache_delete( 'bp_total_groups_for_user_' . $user_id, 'bp' );
}
add_action( 'groups_join_group',    'ships_clear_group_user_object_cache', 10, 2 );
add_action( 'groups_leave_group',   'ships_clear_group_user_object_cache', 10, 2 );
add_action( 'groups_ban_member',    'ships_clear_group_user_object_cache', 10, 2 );
add_action( 'groups_unban_member',  'ships_clear_group_user_object_cache', 10, 2 );
add_action( 'groups_uninvite_user', 'ships_clear_group_user_object_cache', 10, 2 );
add_action( 'groups_remove_member', 'ships_clear_group_user_object_cache', 10, 2 );
 ...
 */
function ships_clear_group_administrator_cache( $group_id ) {
	wp_cache_delete( $group_id, 'bp_group_admins' );
}
add_action( 'groups_promote_member', 'ships_clear_group_administrator_cache' );
add_action( 'groups_demote_member',  'ships_clear_group_administrator_cache' );
add_action( 'groups_delete_group',   'ships_clear_group_administrator_cache' );
 ...
 * using {@link bp_ships_Member::save()}.  Usually via a plugin.
 *
 * @since BuddyPress (2.1.0)
 *
 * @param bp_ships_Member $member
 */
function ships_clear_group_administrator_cache_on_member_save( bp_ships_Member $member ) {
	ships_clear_group_administrator_cache( $member->group_id );
}
add_action( 'groups_member_after_save', 'ships_clear_group_administrator_cache_on_member_save' );
